...
fn main() {
    ...
    //
    // Examine how the flow of execution works.
    // Once you understand it, alter the values passed to the `pause_ms()`
    ...
    // Make two child threads and give them each a receiving end to a channel.  From the
    ...
    let reciver = thread::spawn(move || {
        for val in rx {
            println!("Handler A receive value: {}", val);
        }
    });
